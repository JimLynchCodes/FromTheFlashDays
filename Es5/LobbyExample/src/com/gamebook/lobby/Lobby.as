package com.gamebook.lobby {
	...
	public class Lobby extends MovieClip {
		...
		private var _history:TextField;
		private var _message:TextField;
		private var _joinGame:SimpleButton;
...
//		private var _chatRoomLabel:TextLabel;
		...
		public function Lobby(es:ElectroServer) {
			_es = es;
			trace("es: " + _es);
		...
		public function onCreateOrJoinGameResponse(e:CreateOrJoinGameResponse):void {
			...
			trace("response from quick join gotten");
			
			if (e.successful) {
				trace("response from quick join gotten: success");
				_gameRoom = new Room();
				trace("_room.id " + _room.id);
				_gameRoom.id = _room.id;
				...
				trace(e.error.name);
				trace("response from quick join gotten: error");
		...
		private function onSendClick(e:MouseEvent):void {
			...
			trace("send clicked");
			trace("_message.text.length " + _message.text.length);
			trace("message " + _message.text);
...
//			addChild(_send);
			...
			trace("adding listener to send button");
			
			//used to display the user list
//			_userList = this["userList"];
//			_userList.x = 513;
//			_userList.y = 170;
//			_userList.width = 222;
...
//	/**		//used to launch the create room screen
			_quickJoinGame = this["matchmakingBtn"]; 
...
//				var serverGame:ServerGame = _gameList.selectedItem.data as ServerGame;
				var serverGame:ServerGame = "Jomes" as ServerGame;
				joinGame(serverGame);
//			}
		}
		
		private function onQuickJoinClicked(e:MouseEvent):void {
			trace("quick join clicked");
			quickJoin();
		}
